...
def main():
    ...
    for cid, multiplicity in cid_to_multiplicity.items():
        ...
        res = LcuPrepStatesBenchmark._get_required_data_for_init(
            cid, geometry=None, multiplicity=multiplicity, charge=0
        )

        # Save the result as a list of dictionaries in JSON format

        output_data = {"cid": cid, "multiplicity": multiplicity, "result": res}

        output_file = f"./data/compound_data/output_{cid}.json"
